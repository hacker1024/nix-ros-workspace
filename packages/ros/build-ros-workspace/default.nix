...
let
  buildROSWorkspace =
    ...
    let
      ...
      splitDevPackages = builtins.partition devPackagePredicate packages;
      devPackages = splitDevPackages.right;
      prebuiltPackages = standardPackages ++ splitDevPackages.wrong;

      splitRosPackages = builtins.partition (pkg: pkg.rosPackage or false) (standardPackages ++ packages);
      rosPackages = splitRosPackages.right;
      otherPackages = splitRosPackages.wrong;

      splitRosPrebuiltPackages = builtins.partition (pkg: pkg.rosPackage or false) prebuiltPackages;
...
in
{
  __functor = self: buildROSWorkspace;

  # A higher-level API over buildROSWorkspace that allows specific sets of development and extra packages to be specified.
  explicit = { devPackages, extraPackages, ... }@args: buildROSWorkspace ((builtins.removeAttrs args [ "devPackages" "extraPackages" ]) // {
    packages = extraPackages ++ map (pkg: pkg.overrideAttrs ({ passthru ? { }, ... }: { passthru = passthru // { devPackage = true; }; })) devPackages;
